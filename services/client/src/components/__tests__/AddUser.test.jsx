...
test('AddUser renders properly', () => {
    ...
    expect(element.find('input').get(1).props.name).toBe('firstname');
    expect(element.find('input').get(2).props.name).toBe('lastname');
    expect(element.find('input').get(3).props.name).toBe('address');
    expect(element.find('input').get(4).props.type).toBe('submit');
});
